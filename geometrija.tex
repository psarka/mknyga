%Copyrighted by Vaidotas Juronis, 2011. Some rights reserved.
...
\begin{enumerate} 
  ...
  \item Trikampyje $ABC$ nubrėžė pusiaukraštinę $BD$ ir aukštinę $BE$.
    ...
    %Tarkime, kad $E$ yra tarp $D$ ir $A$. Tada trikampyje $BAD$ aukštinė 
    %sutampa su pusiaukampine, taigi $ED=AE$. Iš čia $ED=\frac{DC}{2}$.
    %Tada iš pusiaukampinės savybės trikampiui $BEC$, $\frac{BE}{BC}=
    %\frac{ED}{DC}=\frac{1}{2}$, taigi stačiajame trikampyje $BEC$ įžambinė
    %dvigubai ilgesnė už statinį. Todėl $\angle BCE=30^\circ$, $\angle CBE= 60^\circ$.
    %Nesunkiai randame, kad $\angle ABC=90^\circ$, $\angle BAC=60^\circ$.
  \item Trikampyje $ABC$ nubrėžė pusiaukampines $AD$ ir $BE$. Pasirodė, 
    kad $\angle BEA=\angle BAE=\angle ADC$. Rasti trikampio kampus.
    %Tegu $\angle BAC=2x=\angle ADC=\angle BEA$. Suskaičiuojame, kad 
    %$$\angle CBA=2\angle ABE=2(180^\circ-2x-2x)=360^\circ-8x.$$ Iš priekampio
    %savybės -- $$2x=\angle CDA=\angle DAB+\angle DBA=360^\circ-7x.$$ Taigi $x=
    %40^\circ$, ir iš čia trikampio kampai yra $\angle A=80^\circ$, $\angle B=
    %40^\circ$, $\angle C=60^\circ$.
  \item Trikampyje $ABC$ ant $BC$ yra taškas $D$ toks, kad $DC=AC=AB$ ir $AD=BD$.
    Rasti trikampio kampus.
    %Pažymėkime $\angle ACB=2x$. Kadangi $AC=DC$, tai $\angle ADC=90^\circ-x$.
    %Kadangi $AC=AB$, tai $\angle ABD=\angle ACB=2x$. Kadangi $AD=BD$, tai
    % $\angle ADB=180-4x$. Tada $180^\circ=\angle ADC+\angle ADB=270^\circ-5x$.
    %Iš čia $x=18^\circ$, ir todėl $\angle A=108^\circ, \angle B=
    %\angle C=36^\circ$.
  \item Trikampyje $ABC$ $BE$ ir $CF$ yra aukštinės, o $D$ yra $BC$ vidurio
    ...
    %Pagal stačiojo trikampio pusiaukraštinės savybę $ED = \frac{CB}{2} =
    %DF$, tad trikampiai $EDC$ ir $BDF$ lygiašoniai. Tuomet $\angle CDE =
    %180^\circ - 2\angle C$ ir $\angle BDF = 180^\circ - 2\angle B$.
    %Kadangi $\angle EDF = 60^\circ$, tai $180^\circ - 2\angle C +
    %180^\circ - 2\angle B = 120^\circ$ ir iš čia $\angle A = 180^\circ
  ...
  \item Ant lygiagretainio $ABCD$ kraštinės $AB$ (arba ant jos tęsinio)
    ...
    %Tegu $MO$ kerta $CD$ taške $N$. Įrodysime, kad trikampiai $MBC$ ir
    %$MND$ yra lygūs. Iš ties $MB=ND$ (nes šios atkarpos simetriškos
    %$O$ atžvilgiu), kraštinė $MN$ bendra abiems, o $\angle DNM = 180^\circ
    %- \angle AMO = 180^\circ - \angle MAD = \angle MBC$.
  ...
  \item Ant trikampio $ABC$ kraštinių $AB, BC, CA$ atitinkamai paimti taškai
    ...
    vienoje tiesėje? 
    %Ne, negali. $AA_1, BB_1, CC_1$ vidurio taškai yra ant trikampio $ABC$
    %vidurio linijų, lygiagrečių atitinkamai kraštinėms $BC, AC, AB$. Šios
    %vidurio linijos sudaro trikampį, o bet kuri linija kerta trikampį
    %daugiausia dviejuose taškuose.
...
\begin{teig}[Kampo tarp stygos ir liestinės savybė] 
...
\begin{sprendimas}
  ...
  Iš čia $AE=FD$.
  %Du apskritimai su bendra styga
\begin{center}
\begin{asy}
import olympiad;
size(200);
...
\item Duotas trikampis $ABC$. Per jo viršūnes $A$ ir $B$ išvestos dvi
  ...
  %Tegu tiesė per tašką $A$ kerta $BC$ taške $A'$, tiesė per $B$ kerta
...
\end{asy}
    ...
    %%parse
    %\begin{itemize}
    %  \item Iš liestinių savybių $AL=AN$. Tada 
    %	\begin{align*}
    %	  AL &= \frac{AL+AN}{2} = \frac{LB+BA+AC+CN}{2} \\
    %	     &= \frac{BM+BA+AC+CM}{2} = \frac{a+b+c}{2} = s.
    %	\end{align*}
    %  \item Skaičiuojame: 
    %	\begin{align*}
    %	  GC &= \frac{FC+GC}{2} = \frac{FC+GC+BF-BE+AG-AE}{2} =\\
    %	     &= \frac{CB+AC-AB}{2} = \frac{a+b-c}{2} = \frac{a+b+c}{2} - c\\
    %	     &= s - c.  
    %	\end{align*}
    %	Taip pat $BM = BL = AL - AB = s - c$, taigi $GC = BM$. Panašiai
    %	gauname ir kad $NC = BE = s - b$ ir $AE = AG = s - a$.
    %  \item Pastebėkime, kad 
    %	$$\angle I_ACN = \frac{\angle BCN}{2} = \frac{\angle CAB + \angle
    %	CBA}{2} = \frac{\angle CBA}{2} + \frac{\angle CAB}{2} = \angle
    %	BII_A,$$ 
    %	taigi $\angle BIA = \angle I_ACA$ ir todėl trikampiai $I_ACA$ ir
    %	$BIA$ yra panašūs pagal $2$ kampus.  Tada $\frac{AB}{AI} =
    %	\frac{AI_A}{AC}$, iš kur gauname $AI \cdot AI_A = AB \cdot AC$. 
    %  \item Pažymėkime $IG=r$ ir pastebėkime, kad $AI_AN$ ir $AIG$ yra
    %	panašūs. Iš čia 
    %	$$S = s*r = AN \cdot IG = AG \cdot I_AN = (s - a) \cdot r_A.$$
    %  \item Reikia įrodyti, kad $S^2 = (s - c)(s - a)(s - b)s$, bet mes
    %	žinome, kad $S = (s - a)r_A$ ir $S = rs$, taigi $S^2 = sr(s -
    %	a)r_A$. Užtenka įrodyti, kad $I_AM \cdot IG = rr_A = (s - b)(s - c)
    %	= MC\cdot CG$, arba kad $\frac{IA_M}{MC} = \frac{CG}{IG}$. Tai
    %	akivaizdu iš trikampių $I_AMC$ ir $IGC$ panašumo: jie abu statūs ir
    %	$$\angle MI_AC = 90^\circ - \angle MCI_A = 90^\circ - \frac{\angle
    %	BAC + \angle CBA}{2} = \frac{\angle BCA}{2} = \angle ICG.$$
    %\end{itemize}
...
Rasti $\angle BDC$.
  ...
  %kertasi taške $O$. Tada 
  %$$\tan \angle BDC= \frac{CE}{DE}= \frac{CE \cdot BE \cdot AE}{BE\cdot AE
  %\cdot DE}=\tan 20^\circ \tan 40^\circ \tan 80^\circ.$$ 
  %Belieka parodyti, kad $\tan 20^\circ \tan 40^\circ \tan 80^\circ=\tan
  %60^\circ$, kas yra vidutinio sunkumo trigonometrijos uždavinys,
  %paliekamas skaitytojui.
  %
...
%   Išvada: kartais trigonometrinis sprendimas yra geriau.
 ...
 %$D$ taško $B$ atžvilgiu, ir tašką $G$, simetrišką taškui $E$ taško $B$
  ...
  %(Atsakymas yra $40^\circ$). Abiejų uždavinių gražumas slypi tame, kad
