...
\begin{thmnr}[Pagrindinė vienetinio apskritimo teorema] Tegu $A$ ir $B$
...
Naudodami kampo išraišką galime lengvai užrašyti tiesės lygtį:

\begin{teig}
  Taškas $z$ priklauso tiesei einančiai per tašką $a$ ir sudarančiai kampą 
  $\theta$ su $Re$ ašimi tada ir tik tada, kai
  $$\frac{z-a}{\bb{z}-\bb{a}}=e^{2i\theta}.$$
\end{teig}

\begin{proof}
  Įrodymas vėl tiesiogiai seka iš kampo išraiškos. Taškas $z$ priklauso norimai
  tiesei tada ir tik tada, kai vektorius $\vv{AZ}$ sudaro kampą $\theta$ arba
  kampą $\theta + \pi$ su realiąja ašimi, o tai ir užrašyta lygybėje.
\end{proof}
